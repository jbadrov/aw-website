    ...
    <div id="tabs">
        <form id="wpc-gateways-form" method="post" action="" style="width: 100%" >
            ...
            <ul style="width: 20%; float: left; margin: 0;">
                <?php
 $i = 0; $active_tab = 0; foreach( (array)$wpc_gateway_plugins as $code => $plugin ) { ?>
                <li>
                <?php
 if ( isset( $wpc_gateways['allowed'] ) && in_array( $code, (array) $wpc_gateways['allowed'] ) ) { if ( !$active_tab ) { $active_tab = $i; } echo '<span id="wpc_ajax_loading_' . $code . '" style="float: left; margin-right: 6px;"></span><input type="checkbox" class="wpc_allowed_gateways" name="wpc_gateway[allowed][]" value="' . $code .'" checked="checked" data-tab_id="' . $i . '" />'; echo ' <a href="' . get_admin_url() . 'admin-ajax.php?action=wpc_get_gateway_setting&plugin=' . $code . '">' . esc_attr($plugin[1]) . '</a><span class="wpc_gateway_title" style="display: none;" style="float:left;white-space:normal;width:80%;">' . esc_attr( $plugin[1] ) . '</span>'; } else { echo '<span id="wpc_ajax_loading_' . $code . '" style="float: left; margin-right: 6px;"></span><input type="checkbox" class="wpc_allowed_gateways" name="wpc_gateway[allowed][]" value="' . $code .'" data-tab_id="' . $i . '" />'; echo ' <a href="' . get_admin_url() . 'admin-ajax.php?action=wpc_get_gateway_setting&plugin=' . $code . '" style="display: none;" >' . esc_attr($plugin[1]) . '</a><span class="wpc_gateway_title"style="float:left;white-space:normal;width:80%;">' . esc_attr( $plugin[1] ) . '</span>'; } $i++; ?>
                </li>
                <?php
 } ?>
...
//                console.log(jQuery( '.wpc_allowed_gateways' ).is(':checked'));
