<?php
 if ( isset( $_POST['update_settings'] ) ) { if ( isset( $_POST['wpc_business_info'] ) ) { $settings = $_POST['wpc_business_info']; } else { $settings = array(); } do_action( 'wp_client_settings_update', $settings, 'business_info' ); do_action( 'wp_client_redirect', admin_url() . 'admin.php?page=wpclients_settings&tab=business_info&msg=u' ); exit; } $wpc_business_info = $this->cc_get_settings( 'business_info' ); ?>
