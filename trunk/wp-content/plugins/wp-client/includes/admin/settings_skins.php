<?php
 if ( isset( $_POST['update_settings'] ) ) { if ( isset( $_POST['wpc_skins'] ) ) { $settings = $_POST['wpc_skins']; } else { $settings = 'light'; } do_action( 'wp_client_settings_update', $settings, 'skins' ); do_action( 'wp_client_redirect', admin_url() . 'admin.php?page=wpclients_settings&tab=skins&msg=u' ); exit; } $wpc_skins = $this->cc_get_settings( 'skins' ); ?>
