<?php
 if ( isset( $_POST['update_settings'] ) ) { if ( isset( $_POST['wpc_general'] ) ) { $settings = $_POST['wpc_general']; } else { $settings = array(); } do_action( 'wp_client_settings_update', $settings, 'general' ); do_action( 'wp_client_redirect', admin_url() . 'admin.php?page=wpclients_settings&msg=u' ); exit; } $wpc_general = $this->cc_get_settings( 'general' ); $wpc_currency = $this->cc_get_settings( 'currency' ); $wpc_custom_titles = $this->cc_get_settings( 'custom_titles' ); ?>
	...
	<div class="postbox">
		...
		<div class="inside">
			<table class="form-table">
                ...
                <tr valign="top">
					<th scope="row">
						<label for="exclude_pp_from_search"><?php printf( __( 'Exclude %s from global Wordpress Search', WPC_CLIENT_TEXT_DOMAIN ), $this->custom_titles['portal']['p'] ) ?>:</label>
					</th>
					<td>
